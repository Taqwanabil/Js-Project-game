...
// nothing when the rocket hits the floor
function check_rotten_rocket_hits_char(rocket) {
    if (collision(rocket, floor)) {
        show_bomb(rocket);
        //decrement_life();
        return true;
...
// to decrecment lifes when hits 
function check_rotten_rocket_hits_char_char(rocket) {
    if (collision(rocket, char)) {
        rocket_top = parseInt(rocket.css('top'));
        if (rocket_top < char_top) {
            decrement_life()
